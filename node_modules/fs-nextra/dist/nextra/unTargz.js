...
async function unTargz(outputDirectory, inputFile, atomic = false) {
    const tar = fs_1.createReadStream(inputFile).pipe(zlib_1.createGunzip()).pipe(new Untar_1.default());
    const writeMethod = atomic ? outputFile_1.default : outputFileAtomic_1.default;
    for await (const { header, file } of tar.files())
        await writeMethod(path_1.resolve(outputDirectory, header.filename), file);
}
exports.default = unTargz;
