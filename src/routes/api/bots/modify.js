...
route.patch("/:id", auth, async (req, res) => {
    ...
    let { long, description, invite, prefix, support, website, github, banner, tags, webhook } = data;

    await Bots.updateOne({ botid: req.params.id }, {$set: { long, description, invite, prefix, support, website, github, banner, tags, webhook, owners: { primary: bot.owners.primary, additional: check.users } } })
